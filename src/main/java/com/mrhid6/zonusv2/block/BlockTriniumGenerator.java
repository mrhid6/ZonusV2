...
public class BlockTriniumGenerator extends BlockMachine implements
		ITileEntityProvider {
	...
	public IIcon getIcon(IBlockAccess par1IBlockAccess, int x, int y, int z,
		...
		if (tileentity instanceof TileEntityTriniumGenerator) {
			orientation = ((TileEntityTriniumGenerator) tileentity)
			...
			if (!world.isRemote) {
				if (world.getTileEntity(x, y, z) instanceof TileEntityTriniumGenerator) {
					player.openGui(ZonusV2.instance,
							GUIs.TRINIUMGENERATOR.ordinal(), world, x, y, z);
				}
			}
			return true;
		}
	...
	public void breakBlock(World world, int x, int y, int z, Block block,
			int par6) {
		TileEntityTriniumGenerator tile = (TileEntityTriniumGenerator) world
				.getTileEntity(x, y, z);
