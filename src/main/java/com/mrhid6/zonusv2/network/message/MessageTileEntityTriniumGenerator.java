...
public class MessageTileEntityTriniumGenerator implements IMessage, IMessageHandler<MessageTileEntityTriniumGenerator, IMessage>{
	...
	public MessageTileEntityTriniumGenerator(TileEntityTriniumGenerator tileEntityZoroFurnace)
	{
	...
	@Override
	public IMessage onMessage(MessageTileEntityTriniumGenerator message, MessageContext ctx)
	{
		TileEntity tileEntity = FMLClientHandler.instance().getClient().theWorld.getTileEntity(message.x, message.y, message.z);

		if (tileEntity instanceof TileEntityTriniumGenerator)
		{
			((TileEntityTriniumGenerator) tileEntity).setOrientation(message.orientation);
			((TileEntityTriniumGenerator) tileEntity).setState(message.state);
			((TileEntityTriniumGenerator) tileEntity).setCustomName(message.customName);
	...
	{
		return String.format("MessageTileEntityTriniumGenerator - x:%s, y:%s, z:%s, orientation:%s, state:%s, customName:%s, owner:%s, power:%s", x, y, z, orientation, state, customName, owner, power);
	}
}
